...
int dynamic1(std::vector<int>& arr)
{
...
int dynamic(int arr[], int size) {
	int bestSoFar=0, bestNow = 0;
	int bestStartIndexSoFar=-1, bestStopIndexSoFar=-1, bestStartIndexNow = -1;	
	int i;
	for( i=0; i < size; i++) {
		int value = bestNow + arr[i];
		if(value > 0)  {
			if (bestNow == 0) {
				bestStartIndexNow = i;
			}
...
int main(int argc, char* argv[]) {
  ...
  while (fgets(linebuff, LINELEN, stdin) != NULL) {
      ...
      switch (function_select) {
         ...
         case 3 : myanswer = dynamic(numarr.data(), numarr.size()); break;
